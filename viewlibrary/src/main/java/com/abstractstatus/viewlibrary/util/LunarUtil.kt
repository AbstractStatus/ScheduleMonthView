...
/**
 ** Created by AbstractStatus at 2021/8/22 10:25.
 */
...
object LunarUtil {
    fun init(context: Context){
        TRUNK_STR =
            context.resources.getStringArray(R.array.trunk_string_array)
        BRANCH_STR =
            context.resources.getStringArray(R.array.branch_string_array)
        SOLAR_TERMS =
            context.resources.getStringArray(R.array.solar_term)
        MONTH_STR =
            context.resources.getStringArray(R.array.lunar_first_of_month)
        TRADITION_FESTIVAL_STR =
            context.resources.getStringArray(R.array.tradition_festival)
        DAY_STR =
            context.resources.getStringArray(R.array.lunar_str)
        SPECIAL_FESTIVAL_STR =
            context.resources.getStringArray(R.array.special_festivals)
        SOLAR_CALENDAR =
            context.resources.getStringArray(R.array.solar_festival)
    }
    ...
    private fun getSolarTerm(year: Int, month: Int, day: Int): String {
        if (!solarTermsMap.containsKey(year)) {
            solarTermsMap[year] = getSolarTerms(year)
        }
    ...
    fun getLunarText(year: Int, month: Int, day: Int): String {
        ...
        val lunar: IntArray = solarToLunar(year, month, day)
    ...
    private fun getSpecialFestival(year: Int, month: Int, day: Int): String {
        ...
        val specialFestivals: Array<String?>? = SPECIAL_FESTIVAL[year]
        val text = year.toString() + getString(month, day)
        var solar = ""
     ...
     * 24节气
     */
    private var solarTermsArray: Array<String>? = null
     ...
     * 黄赤交角系数表
     */
    private val H_C_ANGLE_TABLE = doubleArrayOf(0.0, 50287.92262, 111.24406,
     ...
     * 世界时与原子时之差计算表
     */
    private val DTS = doubleArrayOf(-4000.0, 108371.7, -13036.80, 392.000, 0.0000, -500.0, 17201.0,
            -627.82, 16.170, -0.3413, -150.0, 12200.6, -346.41, 5.403, -0.1593, 150.0, 9113.8, -328.13, -1.647, 0.0377, 500.0, 5707.5, -391.41, 0.915,
            ...
            64.7, -1.66, 5.224, -0.2905, 2150.0, 279.4, 732.95, 429.579, 0.0158, 6000.0)

    /**
     * 离心率
     */
    private val GXC_E = doubleArrayOf(0.016708634, -0.000042037,
            -0.0000001267)

    /**
     * 近点
     */
    private val GXC_P = doubleArrayOf(102.93735 / ANGLE_PER_RAD, 1.71946 / ANGLE_PER_RAD,
            0.00046 / ANGLE_PER_RAD)

    /**
     * 太平黄经
     */
    private val GXC_L = doubleArrayOf(280.4664567 / ANGLE_PER_RAD,
    ...
    private val nutB = doubleArrayOf( // 章动表
            2.1824391966, -33.757045954, 0.0000362262, 3.7340E-08, -2.8793E-10, -171996.0, -1742.0, 92025.0, 89.0, 3.5069406862, 1256.663930738,
            ...
            0.0431251803, -628.301955171, 0.0000026820, 6.5935E-10, 5.5705E-11, -1426.0, 34.0, 54.0, -1.0, 2.3555557435, 8328.691425719, 0.0001545547,
            2.5033E-07, -1.1863E-09, 712.0, 1.0, -7.0, 0.0, 3.4638155059,
            1884.965885909, 0.0000079025, 3.8785E-11, -2.8386E-10, -517.0, 12.0, 224.0, -6.0, 5.4382493597, 16833.175267879, -0.0000874129, 2.7285E-08,
            -2.4750E-10, -386.0, -4.0, 200.0, 0.0, 3.6930589926, 25128.109647645,
            0.0001033681, 3.1496E-07, -1.7218E-09, -301.0, 0.0, 129.0, -1.0,
            3.5500658664, 628.361975567, 0.0000132664, 1.3575E-09, -1.7245E-10, 217.0, -5.0, -95.0, 3.0)
    ...
    private val E13 = doubleArrayOf(0.00000289226, 5.84384198723, 6283.0758499914, 0.00000034955, 0.00000000000, 0.0000000000, 0.00000016819, 5.48766912348, 12566.1516999828)
    private val E14 = doubleArrayOf(0.00000114084, 3.14159265359, 0.0000000000, 0.00000007717, 4.13446589358, 6283.0758499914, 0.00000000765, 3.83803776214, 12566.1516999828)
    ...
    private val E21 = doubleArrayOf(0.00000009030, 3.89729061890, 5507.5532386674, 0.00000006177, 1.73038850355, 5223.6939198022)
    ...
    private val E31 = doubleArrayOf(0.00103018608, 1.10748969588, 6283.0758499914, 0.00001721238, 1.06442301418, 12566.1516999828, 0.00000702215, 3.14159265359, 0.0000000000)
    ...
    private val M12 = doubleArrayOf(0.0048700, -0.0431256817, 628.3019552485, -2.6638815E-06, 6.1639211E-10, -5.4439728E-11, 0.0022800, -27.1705318325, -2.3011998397, 1.5231275E-04, 2.5041111E-07, -1.1863391E-09, 0.0015000, 11.2260974062, 6585.7609102104, -2.1583699E-04, -1.8708058E-07, 9.3204945E-10)
    ...
    private val M1n = doubleArrayOf(3.81034392032, 8.39968473021E+03, -3.31919929753E-05,  //月球平黄经系数
    ...
    private fun Mnn(F: DoubleArray): Double {
        ...
        while (i < F.size) {
            v += (F[i]
                    * sin(F[i + 1] + t1 * F[i + 2] + t2 * F[i + 3] + (t3
                    * F[i + 4]) + t4 * F[i + 5]))
            i += 6
        }
        return v
    ...
    fun getSolarTerms(year: Int): Array<String?> {
        ...
        for (i in 0..18) {
            ...
            q += J2000 + 8.toDouble() / 24 // 计算第i个节气(i=0是春分),结果转为北京时
            val time = setFromJulian(q, true)
            solarTerms[i + 3] = time.toString() + solarTermsMap[i]
        }
        return solarTerms
    }
    ...
    private fun getSolarTermsPreOffset(year: Int): Array<String?> {
        ...
        for (i in 21..23) {
            ...
            val time = setFromJulian(q, true)
            solarTerms[i - 21] = time.toString() + solarTermsMap[i]
        }
        return solarTerms
    }
    ...
    private fun getSolarTermsNextOffset(year: Int): Array<String?> {
        ...
        for (i in 19..20) {
            ...
            q += J2000 + 8.toDouble() / 24 // 计算第i个节气(i=0是春分)
            val time = setFromJulian(q, true)
            solarTerms[i - 19] = time.toString() + solarTermsMap.get(i)
        }
        return solarTerms
    }
    ...
    private val LUNAR_MONTH_DAYS = intArrayOf(1887, 0x1694, 0x16aa, 0x4ad5, 0xab6, 0xc4b7, 0x4ae, 0xa56, 0xb52a, 0x1d2a,
    ...
    private val SOLAR = intArrayOf(1887, 0xec04c, 0xec23f, 0xec435, 0xec649, 0xec83e, 0xeca51, 0xecc46, 0xece3a,
     ...
     */
    private fun solarToLunar(year: Int, month: Int, day: Int): IntArray {
    ...
    fun getTrunkString(year: Int): String? {
        return TRUNK_STR?.get(getTrunkInt(year))
    }
    ...
    fun getBranchString(year: Int): String? {
        return BRANCH_STR?.get(getBranchInt(year))
    }
